...
async function initDb() {
  ...
  const migrations = [
    ...
    `ALTER TABLE users ADD CONSTRAINT users_role_check CHECK (role IN ('parent','teacher','admin','district_admin','coach','athletic_director'))`,
    // ── Operation Pivot: Athletic Department ──────────────────────────────────
    `CREATE TABLE IF NOT EXISTS sports_teams (
      ...
      periods_required INT DEFAULT 4,
      periods_total INT DEFAULT 7,
      created_at TIMESTAMPTZ DEFAULT NOW()
    )`,
    `CREATE TABLE IF NOT EXISTS athlete_profiles (
      id SERIAL PRIMARY KEY,
      student_id INT REFERENCES students(id) ON DELETE CASCADE,
      team_id INT REFERENCES sports_teams(id) ON DELETE CASCADE,
      athlete_id TEXT UNIQUE,
      jersey_number TEXT,
      position TEXT,
      is_active BOOLEAN DEFAULT TRUE,
      eligibility_status TEXT DEFAULT 'eligible' CHECK (eligibility_status IN ('eligible','ineligible','probation','suspended')),
      created_at TIMESTAMPTZ DEFAULT NOW(),
    ...
    `CREATE TABLE IF NOT EXISTS game_events (
      ...
      bus_departure_time TIMESTAMPTZ,
      bus_route_id INT REFERENCES bus_routes(id),
    ...
    `CREATE TABLE IF NOT EXISTS game_day_eligibility (
      id SERIAL PRIMARY KEY,
      game_event_id INT REFERENCES game_events(id) ON DELETE CASCADE,
      student_id INT REFERENCES students(id) ON DELETE CASCADE,
      athlete_profile_id INT REFERENCES athlete_profiles(id),
      is_cleared BOOLEAN,
      periods_attended INT DEFAULT 0,
      periods_required INT NOT NULL,
      periods_total INT NOT NULL,
      conflict_flag BOOLEAN DEFAULT FALSE,
      conflict_type TEXT,
      conflict_data JSONB,
      conflict_resolved BOOLEAN DEFAULT FALSE,
      conflict_resolved_by INT REFERENCES users(id),
      ...
      blocked_reason TEXT,
      created_at TIMESTAMPTZ DEFAULT NOW(),
      UNIQUE (game_event_id, student_id)
    )`,
    `CREATE TABLE IF NOT EXISTS coach_notifications (
      ...
      student_id INT REFERENCES students(id) ON DELETE CASCADE,
      game_event_id INT REFERENCES game_events(id) ON DELETE CASCADE,
      type TEXT NOT NULL CHECK (type IN ('red_flag','cleared','conflict','reminder')),
      message TEXT NOT NULL,
    ...
    `CREATE TABLE IF NOT EXISTS attendance_periods (
      ...
      period_name TEXT,
      status TEXT NOT NULL CHECK (status IN ('present','absent','tardy','excused','unknown')),
      source TEXT DEFAULT 'sis',
      source_id TEXT,
      created_at TIMESTAMPTZ DEFAULT NOW(),
    ...
    `CREATE TABLE IF NOT EXISTS asset_ledger (
      id SERIAL PRIMARY KEY,
      school_id INT REFERENCES schools(id) ON DELETE CASCADE,
      team_id INT REFERENCES sports_teams(id),
      student_id INT REFERENCES students(id),
      ...
      season TEXT,
      graduation_hold BOOLEAN DEFAULT FALSE,
      created_at TIMESTAMPTZ DEFAULT NOW()
    )`,
    `CREATE TABLE IF NOT EXISTS travel_funds (
      id SERIAL PRIMARY KEY,
      game_event_id INT REFERENCES game_events(id) ON DELETE CASCADE,
      school_id INT REFERENCES schools(id) ON DELETE CASCADE,
    ...
    `CREATE TABLE IF NOT EXISTS travel_receipts (
      id SERIAL PRIMARY KEY,
      fund_id INT REFERENCES travel_funds(id) ON DELETE CASCADE,
      submitted_by INT REFERENCES users(id),
      amount NUMERIC(10,2) NOT NULL,
      category TEXT CHECK (category IN ('meals','transportation','lodging','equipment','other')),
      description TEXT,
      receipt_url TEXT,
      approved BOOLEAN,
      approved_by INT REFERENCES users(id),
